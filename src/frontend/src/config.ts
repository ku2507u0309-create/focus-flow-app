...
import { setCurrentPrincipal } from "./mocks/backend";
...
export async function createActorWithConfig(
  ...
  if (mock) {
    // Ensure the mock backend can scope data by principal.
    let principal = "anonymous";
    const maybeIdentity = options?.agentOptions?.identity;
    if (maybeIdentity) {
      try {
        const resolvedIdentity = await Promise.resolve(maybeIdentity as any);
        if (resolvedIdentity?.getPrincipal) {
          principal = resolvedIdentity.getPrincipal().toString();
        }
      } catch {
        // ignore failures and keep anonymous
      }
    }
